...
**PTY 模式**（`--pty`，推荐）：利用 envd 的 PTY 能力，在当前终端中直接开启原生终端会话。无需浏览器或额外的二进制文件。

**Webshell 模式**（默认）：在沙箱内下载并启动 ttyd webshell 服务，然后在浏览器中打开终端。如果沙箱由于网络限制无法从 GitHub 下载 ttyd，可以使用 `--ttyd-binary` 上传本地 ttyd 二进制文件。
