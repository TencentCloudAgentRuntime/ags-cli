...
type fakeControlPlane struct{ calls int }
...
func TestRunProxyUsesLazyRefreshableTokenProvider(t *testing.T) {
	...
	runtime, err := Module().Build(command.Deps{
		...
		DataPlane: RuntimeDeps{
			NewProxy: func(opts dataplaneproxy.Options) (Proxy, error) {
				if cp.calls != 0 {
					t.Fatalf("token acquired before proxy start: calls=%d", cp.calls)
				}
				options = opts
				return fake, nil
			},
	...
	if cp.calls != 1 || acquireErr != nil || acquiredToken != "dpt_secret" {
		t.Fatalf("lazy token = (%q, %v), calls=%d", acquiredToken, acquireErr, cp.calls)
	}
