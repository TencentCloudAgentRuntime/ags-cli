	...
	return command.Module{
		...
		Build: func(deps command.Deps) (command.Runtime, error) {
			...
			return command.Runtime{Handler: command.HandlerFunc(func(ctx context.Context, req command.Request) (*command.Result, error) {
				...
				if response, ok := result.Data.(*ags.ModifyDeploymentResponseParams); ok && response.Deployment != nil {
					result.Effects = append(result.Effects, output.Effect{Kind: "update", Resource: "deployment", Id: stringValue(response.Deployment.DeploymentId)})
